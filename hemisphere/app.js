...
function main() {
  ...
  // hemisphere light(반구광): 천장 색, 바닥 색의 파라미터를 각각 따로 받음. 주로 풍경을 표현할 때 다른 조명과 함께 사용.
  {
    const skyColor = 0xB1E1FF; // 천장 색
    const groundColor = 0xB97A20; // 바닥 색
    const intensity = 1;
    const light = new THREE.HemisphereLight(skyColor, groundColor, intensity);
    scene.add(light);

    // skyColor, groundColor 총 2개의 컬러값을 받아야 하므로, color picker를 2개 생성해줘야 함.
    const gui = new GUI();
    gui.addColor(new ColorGUIHelper(light, 'color'), 'value').name('skyColor'); // HemisphereLight.color는 해당 조명의 skyColor 값을 받는 property임.
